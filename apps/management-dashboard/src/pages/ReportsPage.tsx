...
const LEDGER_TYPE_STYLE: Record<string, { label: string; color: string }> = {
...
export function ReportsPage({ branches, selectedBranchId }: { branches: Branch[]; selectedBranchId: string }) {
  ...
  const presentTypes = Object.keys(LEDGER_TYPE_STYLE).filter((t) => combined[t] !== undefined);
  const breakdownData: BarDatum[] = presentTypes.map((t) => ({
    label: LEDGER_TYPE_STYLE[t].label,
    value: combined[t],
    color: LEDGER_TYPE_STYLE[t].color,
  }));
    ...
    <div className="p-8">
      ...
      <div className="mb-8 grid grid-cols-[repeat(auto-fill,minmax(180px,1fr))] gap-3">
        ...
        {presentTypes
          .filter((t) => t !== 'sale')
          .map((t) => (
            <StatTile key={t} label={LEDGER_TYPE_STYLE[t].label} value={formatMoney(combined[t])} />
          ))}
      </div>

      {breakdownData.length === 0 ? (
        <p className="text-muted">No sales recorded in this date range yet.</p>
      ) : (
