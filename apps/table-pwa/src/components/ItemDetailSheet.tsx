...
export function ItemDetailSheet({ item, onClose }: { item: MenuItem; onClose: () => void }) {
  ...
  const toggleOption = (group: ModifierGroup, option: ModifierOption) => {
    setSelectedByGroup((prev) => {
      ...
      if (isSelected) {
        ...
        next = current; // at cap - ignore extra taps rather than silently dropping an earlier pick
      } else {
        next = [...current, option];
      }
        ...
        <div className="mt-6 flex items-center gap-3">
          <div className="flex items-center rounded-pill border border-border">
            <button
              ...
              className="px-3.5 py-2 text-lg font-medium"
