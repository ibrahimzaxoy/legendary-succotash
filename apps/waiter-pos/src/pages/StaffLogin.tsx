...
// The everyday login: every waiter (and cashier/kitchen/rider, though this
// app only routes waiters past this point) taps their own name on the
// shared tablet, then enters their PIN - fast enough to do between tables.
export function StaffLogin({ device }: { device: Device }) {
      ...
      <div className="flex min-h-screen flex-col items-center justify-center px-6">
        ...
        {submitting ? (
          ...
          <div className="w-full max-w-xs">
            <NumPad
              onDigit={(d) => pin.length < PIN_LENGTH && setPin(pin + d)}
  ...
  if (!options) return <LoadingScreen label="Loading staff…" />;

  return (
    <div className="flex min-h-screen flex-col items-center justify-center px-6">
      <h1 className="mb-1 text-center font-heading text-2xl font-bold">{device.branchName}</h1>
      <p className="mb-8 text-center text-muted">Who’s working?</p>
      <div className="grid w-full max-w-2xl grid-cols-2 gap-4 sm:grid-cols-3">
        ...
        {options.length === 0 && (
          <p className="col-span-full text-center text-muted">
            No waiters set up for this branch yet - add some from the Management Dashboard first.
          </p>
        )}
      </div>
    </div>
